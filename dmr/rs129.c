/* rs129.c — Reed-Solomon (12,9) encoder over GF(256) (rs129.py). */
#include "dmr_internal.h"

static const uint8_t POLY[3] = {64, 56, 14};  /* POLY[0..2] from rs129.py */
static const uint8_t START_MASK[3] = {0x96, 0x96, 0x96};
static const uint8_t END_MASK[3]   = {0x99, 0x99, 0x99};

static const uint8_t EXP_TABLE[512] = {
    
};

static const uint8_t LOG_TABLE[256] = {
    
};

static uint8_t log_mult(uint8_t a, uint8_t b)
{
    if (a == 0 || b == 0)
        return 0;
    return EXP_TABLE[LOG_TABLE[a] + LOG_TABLE[b]];
}

void dmr_rs129_encode(const uint8_t msg[9], uint8_t parity_out[3])
{
    uint8_t parity[3] = {0, 0, 0};
    for (int i = 0; i < 9; i++) {
        uint8_t dbyte = msg[i] ^ parity[2];
        for (int j = 2; j > 0; j--)
            parity[j] = parity[j - 1] ^ log_mult(POLY[j], dbyte);
        parity[0] = log_mult(POLY[0], dbyte);
    }
    /* returns [parity[2], parity[1], parity[0]] */
    parity_out[0] = parity[2];
    parity_out[1] = parity[1];
    parity_out[2] = parity[0];
}

void dmr_rs129_lc_encode(const uint8_t lc[9], int is_term, uint8_t out3[3])
{
    uint8_t parity[3];
    dmr_rs129_encode(lc, parity);
    const uint8_t *mask = is_term ? END_MASK : START_MASK;
    for (int i = 0; i < 3; i++)
        out3[i] = parity[i] ^ mask[i];
}
