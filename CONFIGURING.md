...
| `rssi_trailer` | Optional, default `false`. Send the 75-byte DMRD form — the 53-byte body plus the Homebrew BER/RSSI bytes, HMAC over all 55 — so the c-Bridge's end-of-call RSSI (the CC-CC B-off's `RSSI=`, converted to -dBm) reaches this peer on the VOICE_TERM. **Non-standard: set it only for a peer that expects it** (an hblink3 with `RSSI_TRAILER` on this link). Receiving the 75-byte form needs no switch: it's detected by length and HMAC-verified. |
